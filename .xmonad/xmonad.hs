...
myKeys home conf@(XConfig {terminal = myTerm,
  ...
  mkKeymap conf [-- personal script launcher
                 ...
                 , (prefix "S-f",  runOrRaise "/usr/bin/fbreader"                         (className =? "fbreader"))
                 ...
                 -- shell command
                 , (prefix "C-s",    spawn "/usr/bin/scrot -u $HOME/Pictures/screenshot_$(date +%F_%H-%M-%S).png")
                 , (prefix "M1-s",   spawn "~/bin/touchpad/toggle-touchpad-manual.sh 1; /usr/bin/scrot -s $HOME/Pictures/screenshot_$(date +%F_%H-%M-%S).png")
